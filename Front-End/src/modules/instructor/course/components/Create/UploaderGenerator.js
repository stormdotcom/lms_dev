...
const UploaderGenerator = ({ id, slug, videoNo }) => {
    ...
    function* uploadFileGenerator(file, fileName, slug) {
        try {
            yield dispatch(loaderNotify({ title: "Lecture " + videoNo, message: `${fileName} uploading`, id: `${fileName}` }));
            const startResponse = yield axiosInstance.post(`${API_URL.COURSE.VIDEO}`, {
                fileName: file.name,
                fileType: file.type,
                slug
            });
            ...
            for (let start = 0, partNumber = 1; start < file.size; start += partSize, partNumber++) {
                ...
                const presignedResponse = yield axiosInstance.post(`${API_URL.COURSE.GET_URL}`, {
                    Key,
                    UploadId,
                ...
                const uploadResponse = yield axios.put(uploadUrl, blob, {
            ...
            yield dispatch(successNotify({ message: "File uploaded successfully!" }));
            yield dispatch(sentCompleteVideo({
                Key,
                UploadId,
                Parts: parts,
                courseId: id, slug, videoNo, title: titleInput
            }));
            yield dispatch(dismissNotification(`${fileName}`));
        } catch (error) {
    ...
    const uploadFile = async () => {
        ...
        const generator = uploadFileGenerator(file, fileName, slug);

        const step = (gen) => {
            const next = gen.next();
            if (!next.done) {
                next.value.then((result) => step(gen)).catch((err) => gen.throw(err));
            }
        };
...
export default UploaderGenerator;
